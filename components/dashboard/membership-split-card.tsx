...
const data = [
  { name: "Standard", value: 130, color: "#F20D33" }, // 65%
  { name: "Coaching", value: 50, color: "#2D3748" }, // 25%
  { name: "Supervision", value: 20, color: "#A0AEC0" }, // 10%
];
            ...
            >
              {data.map((entry, index) => (
                <Cell 
                  ...
                  fill={entry.color}
                  style={{
                    filter: activeItem?.name === entry.name ? "brightness(1.1)" : "none",
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-1 transition-all duration-300">
          <span 
            className={`font-teko font-bold leading-none transition-all duration-300 ${
              activeItem ? "text-[36px]" : "text-[32px] text-foreground dark:text-white"
            }`}
